...
import { createBooking, getAccountItems, getBookings, getFavoriteSalons, getPrivacySettings, getProfile, getSalons, getSalonServices, login as loginUser, saveHairstyle, setFavoriteSalon, signup as signupUser, updatePrivacySettings, type AccountItem, type AuthUser, type Booking, type PrivacySettings, type Salon, type SalonService, type UserProfile } from '@/services/api';
import {
  ActivityIndicator,
  Alert,
  Image,
...
function Header({ title, onBack, action, onAction, actionColor = C.ink }: { title: string; onBack?: () => void; action?: keyof typeof Ionicons.glyphMap; onAction?: () => void; actionColor?: string }) {
  return <View style={s.header}>
    ...
    {onAction ? <Pressable accessibilityRole="button" onPress={onAction} style={s.iconButton}>{action && <Ionicons name={action} size={21} color={actionColor} />}</Pressable> : <View style={s.iconButton}>{action && <Ionicons name={action} size={21} color={actionColor} />}</View>}
  </View>;
}
...
export default function FaceFitPrototype() {
  ...
  useEffect(() => {
    ...
    void getFavoriteSalons(authToken).then(setFavoriteSalonIds).catch(() => setFavoriteSalonIds([]));
  }, [authToken]);

  const toggleFavoriteSalon = async (salonId: number) => {
    if (!authToken) {
      Alert.alert('Log in required', 'Log in to save favorite salons.');
      return;
    }
    if (savingFavoriteSalonIds.includes(salonId)) return;
    const wasFavorite = favoriteSalonIds.includes(salonId);
    const salonName = salonRecords.find(salon => salon.id === salonId)?.name || 'Salon';
    setSavingFavoriteSalonIds(ids => [...ids, salonId]);
    setFavoriteSalonIds(ids => wasFavorite ? ids.filter(id => id !== salonId) : [...ids, salonId]);
    try {
      await setFavoriteSalon(authToken, salonId, !wasFavorite);
      setFavoriteToast({ message: wasFavorite ? `${salonName} removed from saved salons` : `${salonName} saved to your favorites`, success: true });
    } catch (error) {
      setFavoriteSalonIds(ids => wasFavorite ? [...new Set([...ids, salonId])] : ids.filter(id => id !== salonId));
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setFavoriteToast(null), 2600);
  ...
  const home = <ScreenFrame><View style={s.topRow}><View><Text style={s.greeting}>Hello, {authUser?.fullName.split(' ')[0] || 'there'}</Text><Text style={s.body}>Ready for a fresh look?</Text></View><Pressable onPress={() => setScreen('notifications')} style={s.avatar}><Ionicons name="notifications-outline" size={21} color={C.roseDark} /></Pressable></View>
    ...
    <SectionTitle title="Salons near you" action="View map" onAction={openCurrentLocationMap} actionLoading={isGettingLocation} />{salonsLoading ? <ActivityIndicator color={C.rose} style={s.dataLoader} /> : salonsError ? <Pressable onPress={loadSalons} style={s.dataState}><Ionicons name="cloud-offline-outline" size={25} color={C.rose} /><Text style={s.cardTitle}>Could not load salons</Text><Text style={s.small}>{salonsError} · Tap to retry</Text></Pressable> : <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.hRow}>{salonRecords.slice(0, 6).map((salon, i) => <Pressable key={salon.id} onPress={() => void openSalon(salon)} style={s.salonCard}><View style={[s.salonThumb, { backgroundColor: i % 2 ? '#ECD7D2' : '#F2E7DF' }]}><Ionicons name="cut-outline" size={28} color={C.rose} /><Pressable accessibilityLabel={`Favorite ${salon.name}`} onPress={(event) => { event.stopPropagation(); void toggleFavoriteSalon(salon.id); }} style={s.salonHeart}><Ionicons name={favoriteSalonIds.includes(salon.id) ? 'heart' : 'heart-outline'} size={20} color={C.rose} /></Pressable></View><Text numberOfLines={1} style={s.cardTitle}>{salon.name}</Text><Text numberOfLines={2} style={s.small}>{salon.address}</Text></Pressable>)}</ScrollView>}
