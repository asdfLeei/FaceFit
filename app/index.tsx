...
export default function FaceFitPrototype() {
  ...
  const home = <ScreenFrame><View style={s.topRow}><View><Text style={s.greeting}>Hello, {authUser?.fullName.split(' ')[0] || 'there'}</Text><Text style={s.body}>Ready for a fresh look?</Text></View><Pressable accessibilityRole="button" accessibilityLabel={`${notificationUnreadCount} unread notifications`} onPress={() => showNotifications('home')} style={s.avatar}><Ionicons name="notifications-outline" size={21} color={C.roseDark} /><NotificationBadge count={notificationUnreadCount} /></Pressable></View>
  ...
  const rolePage = (role: 'stylist' | 'owner', page: string) => {
    ...
    if (owner && page.includes('Dashboard')) {
      const ownerMetrics: { label: string; value: string; caption: string; icon: keyof typeof Ionicons.glyphMap; tone: object }[] = [
        { label: 'Today', value: String(ownerDashboard?.todayBookings ?? 0), caption: 'appointments', icon: 'calendar-clear-outline', tone: s.ownerMetricRose },
        { label: 'Pending', value: String(ownerDashboard?.pendingBookings ?? 0), caption: 'need attention', icon: 'time-outline', tone: s.ownerMetricGold },
        { label: 'Rating', value: (ownerDashboard?.rating ?? 0).toFixed(1), caption: `${ownerDashboard?.reviewCount ?? 0} reviews`, icon: 'star-outline', tone: s.ownerMetricPurple },
        { label: 'Team', value: String(ownerDashboard?.availableStaff ?? 0), caption: 'available staff', icon: 'people-outline', tone: s.ownerMetricGreen },
      ];

      return <ScreenFrame>
        <Header title="Salon dashboard" action="notifications-outline" onAction={() => setScreen('owner-notifications')} actionBadge={notificationUnreadCount} />

        <View style={s.ownerDashboardHero}>
          <View style={s.ownerHeroGlow} />
          <View style={s.ownerHeroTop}>
            <View style={s.ownerHeroStatus}><Ionicons name="checkmark-circle" size={16} color="#BDE8D3" /><Text style={s.ownerHeroStatusText}>Salon active</Text></View>
          </View>
          <View style={s.ownerHeroIdentity}>
            {ownerDashboard?.profileImageUrl ? <Image accessibilityLabel={`${ownerDashboard.name} profile picture`} source={{ uri: getApiAssetUrl(ownerDashboard.profileImageUrl)! }} style={s.ownerDashboardPhoto} /> : <View style={s.ownerDashboardPhotoFallback}><Ionicons name="storefront-outline" size={34} color={C.white} /></View>}
            <View style={s.ownerHeroCopy}>
              <Text style={s.ownerHeroGreeting}>Welcome back,</Text>
              <Text style={s.ownerHeroTitle}>{ownerDashboard?.name || authUser?.fullName || 'Your salon'}</Text>
              <View style={s.ownerHeroMeta}>
                <Ionicons name="location-outline" size={15} color="#F3CCD6" />
                <Text numberOfLines={1} style={s.ownerHeroMetaText}>{ownerDashboard?.address || 'Manage your salon from one place'}</Text>
              </View>
            </View>
          </View>
        </View>

        {ownerDashboardLoading && !ownerDashboard ? <View style={s.ownerDashboardState}><ActivityIndicator color={C.rose} /><Text style={s.small}>Loading live salon data...</Text></View> : ownerDashboardError ? <Pressable onPress={() => void loadOwnerDashboard()} style={s.ownerDashboardState}><Ionicons name="cloud-offline-outline" size={30} color={C.rose} /><Text style={s.cardTitle}>Could not refresh your dashboard</Text><Text style={s.small}>{ownerDashboardError} · Tap to retry</Text></Pressable> : <>
          <View style={s.ownerSectionHeading}>
            <View><Text style={s.ownerSectionEyebrow}>PERFORMANCE</Text><Text style={s.ownerSectionTitle}>Today at a glance</Text></View>
          </View>

          <View style={s.ownerMetricsGrid}>
            {ownerMetrics.map(metric => <View key={metric.label} style={[s.ownerMetricCard, { flexBasis: isDesktopWeb ? '22%' : '46%' }]}>
              <View style={[s.ownerMetricIcon, metric.tone]}><Ionicons name={metric.icon} size={21} color={C.roseDark} /></View>
              <Text style={s.ownerMetricValue}>{metric.value}</Text>
              <Text style={s.ownerMetricLabel}>{metric.label}</Text>
              <Text style={s.ownerMetricCaption}>{metric.caption}</Text>
            </View>)}
          </View>

          <View style={s.ownerSectionHeading}>
            <View><Text style={s.ownerSectionEyebrow}>QUICK ACCESS</Text><Text style={s.ownerSectionTitle}>Manage your salon</Text></View>
          </View>

          <View style={s.ownerManagementGrid}>
            {nav.map((item, index) => <Pressable
              accessibilityRole="button"
              key={item.label}
              onPress={() => setScreen(item.target)}
              style={({ pressed }) => [s.ownerManagementCard, { flexBasis: isDesktopWeb ? '30%' : '100%' }, pressed && s.pressed]}
            >
              <View style={[s.ownerManagementIcon, index % 3 === 1 && s.ownerMetricGold, index % 3 === 2 && s.ownerMetricGreen]}><Ionicons name={item.icon} size={22} color={C.roseDark} /></View>
              <View style={s.ownerManagementCopy}>
                <Text style={s.ownerManagementTitle}>{item.label}</Text>
                <Text style={s.ownerManagementDescription}>{index === 0 ? `${ownerDashboard?.pendingBookings ?? 0} requests waiting` : index === 1 ? `${ownerDashboard?.serviceCount ?? 0} published services` : index === 2 ? `${ownerDashboard?.availableStaff ?? 0} team members available` : index === 3 ? 'Update salon information' : `${ownerDashboard?.reviewCount ?? 0} customer reviews`}</Text>
              </View>
              <Ionicons name="arrow-forward" size={18} color={C.rose} />
            </Pressable>)}
          </View>

          <View style={s.ownerHealthPanel}>
            <View style={s.ownerHealthIcon}><Ionicons name="shield-checkmark" size={24} color={C.white} /></View>
            <View style={s.ownerManagementCopy}><Text style={s.ownerHealthTitle}>Your salon is ready for bookings</Text><Text style={s.ownerHealthText}>{ownerDashboard?.serviceCount ?? 0} services are visible to FaceFit customers.</Text></View>
            <Ionicons name="sparkles-outline" size={24} color="#8EBFA9" />
          </View>
        </>}
      </ScreenFrame>;
    }
